...
def send_email_notification(level: str, service: str, metric: str, value: str, rule: str):
    print("\n" + "📬 " + "=[ PROTOCOLO SMTP - NOTIFICAÇÃO DE ALERTA ]=".center(60, "="))
    print(f" From:    netwatch-alertas@idp.edu.br")
    print(f" To:      equipe-infra-suporte@idp.edu.br")
    print(f" Subject: [{level}] Alerta de Indisponibilidade no Ativo: {service.upper()}")
    print("-" * 64)
    print(f" Mensagem de Emergência:")
    print(f" O sistema NetWatch detectou uma anomalia operacional severa.")
    print(f" O serviço [{service.upper()}] quebrou o limiar operacional:")
    print(f"  -> Métrica Violada: {metric}")
    print(f"  -> Valor Registrado: {value}")
    print(f"  -> Diagnóstico da Regra: {rule}")
    print(f"\n Status: TRANSMITIDO. Log de envelope SMTP renderizado com sucesso.")
    print("=" * 64 + "\n")
